...
> **Versión:** 1.11 · **Nombre:** Nilo Explorador (antes "Mate Aventura") · **Estado:** Prototipo funcional + PWA instalable (publicable como app web) · **Áreas:** Números (matemáticas) y Palabras (vocabulario inglés ↔ español)
...
| 🔥 **Fito** | El fuego (área Seguridad) | Amable con reglas; fuerte si nadie lo cuida; no es enemigo | "Puedo ayudarte… pero nunca debes jugar conmigo" | `Fito.png` (avatar recortado de `fito_personaje.png`; respaldo SVG) | ✅ Implementado |
| 👨‍🚒 **Bruno** | El bombero (área Seguridad) | Tranquilo, amable, ayuda a todos | "Si ves fuego, me llamas al 911" | `Bruno.png` (avatar recortado de `bruno_personaje.png`; respaldo SVG) | ✅ Implementado |
...
- **Números 🔢** — sumas y restas (el contenido original).
- **Palabras 🗣️** — traducción **español ↔ inglés** por **opción múltiple** (4 botones). Niveles: *Básico* (palabra **con dibujo/emoji** de ayuda, siempre es→en), *Medio* (solo la palabra, dirección al azar) y *Mezcla*.
- **Seguridad 🔥** — lección guiada sobre el fuego y los incendios (ver 5.d).
...
## 5.d Área Seguridad — incendios (Fito y Bruno)

Lección **guiada e interactiva** (no es el formato de 6 operaciones), con el enfoque de la app: claro, visual, **sin generar miedo**, con reglas y refuerzo positivo. El objetivo no es temer al fuego, sino reconocer cuándo es útil, cuándo es peligroso y qué hacer.

- **Personajes**: 🔥 **Fito el Fuego** (amable con reglas) y 👨‍🚒 **Bruno el Bombero** (tranquilo y amable). Dibujados en **SVG**; sustituibles por `Fito.png` / `Bruno.png`.
- **Flujo (`FIRE_STEPS`, 26 pasos)** que cubre los 9 niveles del plan + el juego de escenarios: dónde hay fuego y si hay un adulto, para qué sirve, qué no hacer ("esto hace crecer a Fito demasiado"), qué es un incendio (humo→fuego), qué hacer (👀 veo humo → 🚶 salgo → 🧑 adulto → 📞 911 → 🚒 bomberos), **nunca regresar**, el peligro del **humo**, avanzar agachado, y los bomberos.
- **Interacción**: tarjetas con "Siguiente" y preguntas de opción; acierto = refuerzo y avanza, error = pista amable y se reintenta (sin castigo).
- **Cierre**: celebración de Fito y Bruno + **insignia "Seguro con el fuego"** 🚒 (`fireDone`).

> Se entra eligiendo el área **Seguridad 🔥** y "Empezar". El módulo es ampliable añadiendo objetos a `FIRE_STEPS`.
...
- **Pegatinas especiales de personaje** (en el Álbum, sección "🌟 Especiales"): **Fito** y **Bruno** se descubren al completar el reto de **Seguridad**; **Bastián**, al desbloquear su primera construcción. Muestran la **cara del personaje** (la lámina ahora admite imágenes) y no salen en misiones normales.
...
| Área Seguridad: incendios (Fito y Bruno), 9 niveles + escenarios 🔥 | ✅ |
| Imágenes propias de Fito y Bruno | ✅ |
...
- `Fito.png`, `Bruno.png` — avatares de los personajes del área Seguridad (recortados de las imágenes completas; si faltan, se usan sus dibujos SVG).
- `fito_personaje.png`, `bruno_personaje.png` — imágenes oficiales de cuerpo completo de Fito y Bruno.
